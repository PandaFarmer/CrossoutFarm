...
class ObjectDetector:
    ...
    def predict_boxes(self, feature_maps: torch.Tensor) -> np.ndarray:
        ...
        for batch_idx in range(feature_maps.shape[0]):
            boxes = []
            scores = []
            class_ids = []
            ...
            for y in range(grid_size[0]):
                for x in range(grid_size[1]):
                    ...
                    for anchor_idx, anchor in enumerate(self.anchors):
                        # Get predictions for this cell/anchor combination
